...
# title = form.plainTextEdit_2.toPlainText()
# body = form.plainTextEdit.toPlainText()
...
def on_click(number):
    global calc_date, start_date
    start_date = now_date

    calc_date = form.calendarWidget.selectedDate()
    # description = form.plainTextEdit.toPlainText()

    if form.pushButton_6.clicked.connect(on_click):
        f.show("all")
    if form.pushButton.clicked.connect(on_click):
        f.add()
    if form.pushButton_5.clicked.connect(on_click):
        f.show("all")
        f.id_edit_del_show("del")
    if form.pushButton_4.clicked.connect(on_click):
        f.show("all")
        f.id_edit_del_show('edit')
    if form.pushButton_7.clicked.connect(on_click):
        f.show('date')
    if form.pushButton_8.clicked.connect(on_click):
        f.show("id")
        f.id_edit_del_show("show")
    if form.pushButton_9.clicked.connect(on_click):
        tracker.goodbuy()
